...
from tinygrad.codegen.assembly import AssemblyLanguage, Register, uops_to_asmstyle
...
def specialize_to_rdna(function_name, lang) -> str:  #Tuple[str, str]:
  ...
  def assemble(args, ins, v_cnt, s_cnt):
...
def uops_to_rdna_asm(function_name:str, uops:List[UOp]) -> Tuple[str, List[int], List[int], bool]:
  ...
  return specialize_to_rdna(function_name, lang), global_size[::-1], local_size[::-1], True 